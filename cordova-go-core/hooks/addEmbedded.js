...
module.exports = function(context) {
  ...
  function getFileIdAndRemoveFromFrameworks(myProj, fileBasename) {
    var fileId = "";
    const pbxFrameworksBuildPhaseObjFiles = myProj.pbxFrameworksBuildPhaseObj(
    ...
    for (var i = 0; i < pbxFrameworksBuildPhaseObjFiles.length; i++) {
      ...
      if (
        frameworkBuildPhaseFile.comment &&
        frameworkBuildPhaseFile.comment.indexOf(fileBasename) != -1
      ) {
  ...
  function addRunpathSearchBuildProperty(proj, build) {
    ...
    if (!LD_RUNPATH_SEARCH_PATHS) {
      ...
      LD_RUNPATH_SEARCH_PATHS.indexOf("@executable_path/Frameworks") == -1
    ) {
      var newValue = LD_RUNPATH_SEARCH_PATHS.substr(
        0,
  ...
  const pluginPathInPlatformIosDir =
    projectName + "/Plugins/" + context.opts.plugin.id;

  process.chdir("./platforms/ios");
  const frameworkFilesToEmbed = fromDir(
